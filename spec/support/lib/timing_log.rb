module LogfileInterval
  module LineParser
    class TimingLog < Base
      ...
      add_column :name => :action,       :pos => 2, :agg_function => :count
    ...
    class TimingLogWithGrouping < Base
      ...
      add_column :name => :timestamp,    :pos => 1, :agg_function => :timestamp
      add_column :name => :action,       :pos => 2, :agg_function => :count,     :group_by => :action
      add_column :name => :total_time,   :pos => 3, :agg_function => :average,   :group_by => :action, :conversion => :integer
      add_column :name => :num_bytes,    :pos => 4, :agg_function => :sum,       :group_by => :action, :conversion => :integer
      add_column :name => :rss,          :pos => 5, :agg_function => :delta,     :group_by => :action, :conversion => :float
